...
export async function getPlayerInfo(playerId: number, season: string = "2024-25") {
  const res = await fetch(`${BASE_URL}/api/players/${playerId}/info?season=${season}`)
  return res.json()
}

export async function getShots(playerId: number, season: string = "2024-25", gameId?: string) {
  const url = gameId
...
export async function getGameLog(playerId: number, season: string = "2024-25") {
  const res = await fetch(`${BASE_URL}/api/players/${playerId}/games?season=${season}`)
  return res.json()
}

export async function analyzePlayer(playerId: number, season: string = "2024-25") {
